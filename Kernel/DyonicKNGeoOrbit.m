...
BeginPackage["DyonicKNGeodesics`DyonicKNGeoOrbit`"];


DyonicKNGeoOrbit::usage = "For generic Orbits and returns a DyonicKNGeoOrbitFunction[..] which stores the orbital trajectory and parameters.";
DyonicKNGeoOrbitFunction::usage = "DyonicKNGeoOrbitFunction[a,assoc] an object for storing the trajectory and orbital parameters in the assoc Association.";
Begin["`Private`"];
...
DyonicKNGeoOrbit::r0outofboundsGen = "Intial radius `1` is not between the outer radial root `2`, and inner radial root `3`.";
DyonicKNGeoOrbit::\[Theta]0outofbounds = "Intial polar angle `1` must be between the ranges (`2`,`3`) or (-`2`,-`3`)";
DyonicKNGeoOrbit::TooMuchENergy = "the total value of \!\(\*SuperscriptBox[\(\[ScriptCapitalO]\), \(2\)]\)+\!\(\*SuperscriptBox[\(a\), \(2\)]\) = `1` is greater than one your are breakig cosmic censorship, stop this!"


Options[DyonicKNGeoRealOrbitMino] = {"Roots"-> Automatic};
DyonicKNGeoRealOrbitMino[a_, \[ScriptCapitalO]_ , \[CapitalOmega]_, \[Delta]_ , \[ScriptCapitalE]_, \[ScriptCapitalL]_, \[ScriptCapitalK]_, initCoords_, OptionsPattern[Options[DyonicKNGeoRealOrbitMino]]] := Module[
	...
	If[r0<=r3||r0>=r4,
		Message[DyonicKNGeoOrbit::r0outofboundsGen,r0,r4,r3];
		Return[$Failed]
		];
	If[Abs[\[Theta]0]<Abs[ArcCos[Z1]]||Abs[\[Theta]0]>\[Pi] - Abs[ArcCos[Z1]],
		Message[DyonicKNGeoOrbit::\[Theta]0outofbounds,\[Theta]0,Abs[ArcCos[Z1]],\[Pi]-Abs[ArcCos[Z1]] ];
		Return[$Failed]
		];
...
(*Note: If including Precission Error May occur in the case of evaluating R2INT\[Lambda][0] due to a bug with mathematica in
 evaluating terms of the form EllipticE[0, num`n], num is any number and n is less thna machine precision*)
	...
	DyonicKNGeoOrbitFunction[a ,\[ScriptCapitalO], \[CapitalOmega], \[Delta], \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalK], assoc]
]
...
DyonicKNGeoOrbit::fourcomplexroots = "This function can only currently solve plunging geodesics in the paramater space where we have four finite real roots or two finite real roots and two finite complex roots.";
DyonicKNGeoOrbit::highenergy = "This code does not currently support plunging solutions with \[ScriptCapitalE]>1, \[ScriptCapitalE]=`1` has been selected ";
DyonicKNGeoOrbit::negativecarter = "This code does not currently support plunging solutions with Q<0, Q=`1` has been selected ";
DyonicKNGeoOrbit::rIoutbounds = "Given ISSO radius `1` is not in the allowed range between `2` and `3` for spin `4`.";
DyonicKNGeoOrbit::Inclinationoutofbounds = "Given inclination angle `1` is not between -\!\(\*FractionBox[\(\[Pi]\), \(2\)]\) and \!\(\*FractionBox[\(\[Pi]\), \(2\)]\)."
DyonicKNGeoOrbit::aoutofbounds1 = "The Orbits packages does not currently support the extremal a=1 case"
DyonicKNGeoOrbit::aoutofbounds0 = "The Orbits packages does not currently support the zero spin a=0 case"
DyonicKNGeoOrbit::NoBoundMotion = "The Paramaters you provided do not correspond to bound motion"


DyonicKNGeoOrbit[a_:0.7 ,\[ScriptCapitalO]_:0.3, \[CapitalOmega]_:3, \[Delta]_:3,\[ScriptCapitalE]_:0.8,\[ScriptCapitalL]_:0.3,\[ScriptCapitalK]_:3, initCoords:{_,_,_,_}:Automatic,OptionsPattern[]]:=Module[
	{param, method, RI, DN, RISSOMIN, RISSOMAX, ROOTS, RealRoots,ComplexRoots},
		
	If[a==1,
		Message[DyonicKNGeoOrbit::aoutofbounds1];
		Return[$Failed]
		];
	
	If[\[ScriptCapitalE]>=1,
		Message[DyonicKNGeoOrbit::highenergy,\[ScriptCapitalE]];
		Return[$Failed]
		];
		
	If[\[ScriptCapitalK]<0,
		Message[DyonicKNGeoOrbit::negativecarter,\[ScriptCapitalK]];
		Return[$Failed]
		];
	...
	If[Length[ComplexRoots]== 2, Message[DyonicKNGeoOrbit::NoBoundMotion]; Return[$Failed]];
	If[Length[ComplexRoots]== 0, Return[DyonicKNGeoRealOrbitMino[a,\[ScriptCapitalO], \[CapitalOmega], \[Delta], \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalK], initCoords,"Roots"->ROOTS]]];
	];


DyonicKNGeoOrbitFunction /:
 MakeBoxes[kgof:DyonicKNGeoOrbitFunction[a_,\[ScriptCapitalO]_,\[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_,\[ScriptCapitalL]_,\[ScriptCapitalK]_,assoc_], form:(StandardForm|TraditionalForm)] :=
...
DyonicKNGeoOrbitFunction[a_,\[ScriptCapitalO]_,\[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_,\[ScriptCapitalL]_,\[ScriptCapitalK]_,assoc_][\[Lambda]_/;StringQ[\[Lambda]] == False] := Through[assoc["Trajectory"][\[Lambda]]]
DyonicKNGeoOrbitFunction[a_,\[ScriptCapitalO]_,\[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_,\[ScriptCapitalL]_,\[ScriptCapitalK]_,assoc_][y_?StringQ] := assoc[y]
Keys[g_DyonicKNGeoOrbitFunction]^:=Keys[g[[5]]]
