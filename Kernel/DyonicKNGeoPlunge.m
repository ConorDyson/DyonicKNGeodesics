...
(* ::Title:: *)
(*DyonicKNGeoOrbit subpackage of DyonicKNGeodesics*)
...
BeginPackage["DyonicKNGeodesics`DyonicKNGeoPlunge`"];


DyonicKNGeoPlunge::usage = "Takes either DyonicKNGeoPlunge[a, {\[ScriptCapitalO],\[CapitalOmega],\[Delta], \[ScriptCapitalE],\[ScriptCapitalL],\[ScriptCapitalQ]}]";
DyonicKNGeoPlungeFunction::usage = "DyonicKNGeoPlungeFunction[...] an object for storing the trajectory and orbital parameters in the assoc Association.";
Begin["`Private`"];
...
DyonicKNGeoPlunge::r0outofboundsGen = "Intial radius `1` is not between the outer radial root `2`, and inner radial root `3`.";
DyonicKNGeoPlunge::\[Theta]0outofbounds = "Intial polar angle `1` must be between the ranges (`2`,`3`) or (-`2`,-`3`)";
DyonicKNGeoPlunge::TooMuchENergy = "the total value of \!\(\*SuperscriptBox[\(\[ScriptCapitalO]\), \(2\)]\)+\!\(\*SuperscriptBox[\(a\), \(2\)]\) = `1` is greater than one your are breakig cosmic censorship, stop this!"


Options[DyonicKNGeoRealPlungeMino] = {"Roots"-> Automatic};
DyonicKNGeoRealPlungeMino[a_, \[ScriptCapitalO]_ , \[CapitalOmega]_,\[Delta]_, \[ScriptCapitalE]_, \[ScriptCapitalL]_, \[ScriptCapitalK]_, initCoords_, OptionsPattern[Options[DyonicKNGeoRealPlungeMino]]] := Module[
	...
	If[\[ScriptCapitalO]^2+a^2 >1,
		Message[DyonicKNGeoPlunge::TooMuchENergy,\[ScriptCapitalO]^2+a^2];
		Return[$Failed]
		];
	...
	If[r0<=r4||r0>=r3,
		Message[DyonicKNGeoPlunge::r0outofboundsGen,r0,r3,r4];
		Return[$Failed]
		];
	If[Abs[\[Theta]0]<Abs[ArcCos[Z1]]||Abs[\[Theta]0]>\[Pi] - Abs[ArcCos[Z1]],
		Message[DyonicKNGeoPlunge::\[Theta]0outofbounds,\[Theta]0,Abs[ArcCos[Z1]],\[Pi]-Abs[ArcCos[Z1]] ];
		Return[$Failed]
		];
...
(*Note: If including Precission Error May occur in the case of evaluating R2INT\[Lambda][0] due to a bug with mathematica in
 evaluating terms of the form EllipticE[0, num`n], num is any number and n is less thna machine precision*)
	...
	DyonicKNGeoPlungeFunction[a,\[ScriptCapitalO], \[CapitalOmega],\[Delta], \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalK], assoc]
]
...
DyonicKNGeoComplexPlungeMino[a_ , \[ScriptCapitalO]_ , \[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_, \[ScriptCapitalL]_, \[ScriptCapitalK]_, initCoords_, OptionsPattern[Options[DyonicKNGeoComplexPlungeMino]]] := Module[{consts,MCPP, MCPM, MCMP , MCMM , assoc,MinoR,\[ScriptCapitalQ],m,z1,z2,z3,z4,\[CapitalUpsilon]r,\[CapitalUpsilon]\[Theta],t0,r0,\[Phi]0,M,D1M,D1P,D2M,D2P,Minoz,MinozFunc,MinoRFunc,\[Theta]0,z0,e,b,c,d,A,B,chi,kr,p2,f, t, r,z, \[Theta], J,\[Phi], R1,R2,R3,R4, RM, RP,ROOTS,RealRoots,ComplexRoots,kz,Z1, Z2, AMR,CNR,SNR,DNR,AMZ},
	...
	If[\[ScriptCapitalO]^2+a^2 >1,
		Message[DyonicKNGeoPlunge::TooMuchENergy,a^2+\[ScriptCapitalO]^2];
		Return[$Failed]
		];
	...
	If[r0<R1||r0>R2,
		Message[DyonicKNGeoPlunge::r0outofboundsGen,r0,R2,R1];
		Return[$Failed]
		];
	If[Abs[\[Theta]0]<Abs[ArcCos[Z1]]||Abs[\[Theta]0]>\[Pi] - Abs[ArcCos[Z1]],
		Message[DyonicKNGeoPlunge::\[Theta]0outofbounds,\[Theta]0,Abs[ArcCos[Z1]],\[Pi]-Abs[ArcCos[Z1]] ];
...
DyonicKNGeoPlunge::r0outofboundsGen = "Intial radius `1` is not between the outer radial root `2`, and inner radial root `3`.";
DyonicKNGeoPlunge::\[Theta]0outofbounds = "Intial polar angle `1` must be between the ranges (`2`,`3`) or (-`2`,-`3`)";
DyonicKNGeoPlunge::TooMuchENergy = "the total value of \!\(\*SuperscriptBox[\(\[ScriptCapitalO]\), \(2\)]\)+\!\(\*SuperscriptBox[\(a\), \(2\)]\) = `1` is greater than 1 your are breaking cosmic censorship, stop this!"


(*Options[DyonicKNGeoRealBoundMino] = {"Roots"-> Automatic};
DyonicKNGeoRealBoundMino[a_, \[ScriptCapitalE]_, \[ScriptCapitalL]_, \[ScriptCapitalK]_, \[ScriptCapitalO]_ , \[CapitalOmega]_, initCoords_, OptionsPattern[Options[DyonicKNGeoRealBoundMino]]] := Module[
	...
	DyonicKNGeoPlungeFunction[a, \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalQ], assoc]
]
*)


(* ::Section:: *)
(*DyonicKNGeoOrbit and DyonicKNGeoOrbitFuction*)


DyonicKNGeoPlunge::fourcomplexroots = "This function can only currently solve plunging geodesics in the paramater space where we have four finite real roots or two finite real roots and two finite complex roots.";
DyonicKNGeoPlunge::highenergy = "This code does not currently support plunging solutions with \[ScriptCapitalE]>1, \[ScriptCapitalE]=`1` has been selected ";
DyonicKNGeoPlunge::negativecarter = "This code does not currently support plunging solutions with Q<0, Q=`1` has been selected ";
DyonicKNGeoPlunge::rIoutbounds = "Given ISSO radius `1` is not in the allowed range between `2` and `3` for spin `4`.";
DyonicKNGeoPlunge::Inclinationoutofbounds = "Given inclination angle `1` is not between -\!\(\*FractionBox[\(\[Pi]\), \(2\)]\) and \!\(\*FractionBox[\(\[Pi]\), \(2\)]\)."
DyonicKNGeoPlunge::aoutofbounds1 = "The plunges packages does not currently support the extremal a=1 case"
DyonicKNGeoPlunge::aoutofbounds0 = "The plunges packages does not currently support the zero spin a=0 case"


DyonicKNGeoPlunge[a_:0.7 ,\[ScriptCapitalO]_:0.3, \[CapitalOmega]_:3, \[Delta]_:3,\[ScriptCapitalE]_:0.8,\[ScriptCapitalL]_:0.3,\[ScriptCapitalK]_:3, initCoords:{_,_,_,_}:Automatic,OptionsPattern[]]:=Module[
	{param, method, RI, DN, RISSOMIN, RISSOMAX, ROOTS, RealRoots,ComplexRoots},
		
	If[a==1,
		Message[DyonicKNGeoPlunge::aoutofbounds1];
		Return[$Failed]
		];
	
	If[\[ScriptCapitalE]>=1,
		Message[DyonicKNGeoPlunge::highenergy,\[ScriptCapitalE]];
		Return[$Failed]
		];
		
	If[\[ScriptCapitalK]<0,
		Message[DyonicKNGeoPlunge::negativecarter,\[ScriptCapitalK]];
		Return[$Failed]
		];
	...
	If[Length[ComplexRoots]== 2, Return[DyonicKNGeoComplexPlungeMino[a,\[ScriptCapitalO], \[CapitalOmega] ,\[Delta], \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalK], initCoords,"Roots"->ROOTS]]];
	If[Length[ComplexRoots]== 0, Return[DyonicKNGeoRealPlungeMino[a,\[ScriptCapitalO], \[CapitalOmega] ,\[Delta], \[ScriptCapitalE], \[ScriptCapitalL], \[ScriptCapitalK], initCoords,"Roots"->ROOTS]]];
	];


DyonicKNGeoPlungeFunction /:
 MakeBoxes[kgof:DyonicKNGeoPlungeFunction[a_,\[ScriptCapitalO]_,\[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_,\[ScriptCapitalL]_,\[ScriptCapitalK]_,assoc_], form:(StandardForm|TraditionalForm)] :=
...
DyonicKNGeoPlungeFunction[a_,\[ScriptCapitalO]_,\[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_,\[ScriptCapitalL]_,\[ScriptCapitalK]_,assoc_][\[Lambda]_/;StringQ[\[Lambda]] == False] := Through[assoc["Trajectory"][\[Lambda]]]
DyonicKNGeoPlungeFunction[a_,\[ScriptCapitalO]_,\[CapitalOmega]_,\[Delta]_,\[ScriptCapitalE]_,\[ScriptCapitalL]_,\[ScriptCapitalK]_,assoc_][y_?StringQ] := assoc[y]
Keys[g_DyonicKNGeoPlungeFunction]^:=Keys[g[[5]]]
